  ...
  object : Migration(20, 21) {
    override fun migrate(db: SupportSQLiteDatabase) {
      ...
      db.execSQL("""CREATE TABLE IF NOT EXISTS `audiobookshelf_servers` (
        ...
        `serverUrl` TEXT NOT NULL, `username` TEXT NOT NULL, `token` TEXT NOT NULL,
        `userId` TEXT NOT NULL, `activeLibraryId` TEXT, `lastConnected` INTEGER NOT NULL)""")
    }
  }
...
val DatabaseModule =
  module {
    ...
    single { get<MpvInfinityDatabase>().audiobookDao() }
    single { get<MpvInfinityDatabase>().audiobookshelfServerDao() }
    single {
      app.infinity.mpvz.data.audiobookshelf.AudiobookshelfClient(
        httpClient = get(),
        json = get(),
      )
    }
    single {
      app.infinity.mpvz.repository.AudiobookshelfRepository(
        dao = get(),
        client = get(),
      )
    }

...
internal abstract class VisualizerTextureView(context: Context) : TextureView(context), TextureView.SurfaceTextureListener {
  ...
  private class RenderSession(
    ...
    fun setRenderingEnabled(enabled: Boolean) {
      handler.post {
        ...
        if (enabled) requestFrame() else choreographer?.removeFrameCallback(frameCallback)
      }
    }
...
internal fun VisualizerTextureView.startVisualizerRendering() {
